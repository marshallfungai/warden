...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Warden - Zero-downtime Deployment Orchestrator"
    )
    subparsers = parser.add_subparsers(dest="command", help="Commands")

    subparsers.add_parser("run", help="Run Warden")
    deploy_parser = subparsers.add_parser(
        "deploy", help="Deploy a new version of the service"
    )
    deploy_parser.add_argument("version", help="The version of the service to deploy")
    rollback_parser = subparsers.add_parser(
        "rollback", help="Rollback to the previous version of the service"
    )
    status_parser = subparsers.add_parser(
        "status", help="Show the current active snapshot of the service"
    )
...
def run_warden():
    ...
    registry_watcher = RegistryWatcher(
        image_name=os.getenv("IMAGE_NAME", "demo-app"),
...
def start_webhook_server(port: int = 5000):
    ...
    webhook_server = WebhookServer(
        port=port, on_deploy=deploy_service, on_rollback=rollback_service
    )
    webhook_server.run()
